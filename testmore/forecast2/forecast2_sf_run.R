...
library(Matrix)
library(gridExtra)
fit<- fitfromweb("WBSS_HAWG_2019_sf")
source("stockassessment/R/forecast2_sf.R")
...
FC[[length(FC)+1]] <- forecast2(fit, fscale=c(1,NA,0,0,0,0,0,0,0,0,0), 
                                catchval=c(NA,23367,NA,NA,NA,NA,NA,NA,NA,NA,NA),
                                fval=c(NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA),
                                rec.years=2013:2017, 
                                label="F=0",
                                nosim=nsim, deterministic=FALSE,
                                Fscenario=1,
                                #MSYBtrig=c(NA, NA, MSYBtrig, MSYBtrig, MSYBtrig, MSYBtrig, MSYBtrig, MSYBtrig, MSYBtrig, MSYBtrig, MSYBtrig), 
                                #Blim=c(NA, NA, Blim, Blim, Blim, Blim, Blim, Blim, Blim, Blim, Blim),
                                Fmsy=Fmsy,
...
FC[[length(FC)+1]] <- forecast2(fit, fscale=c(1,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA), 
                                catchval=c(NA,23367,23367,23367,23367,23367,23367,23367,23367,23367,23367),
                                fval=c(NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA),
                                rec.years=2013:2017, 
...
save.image(paste0(name_sim,".RData"))
...
pdf(file = paste0(name_sim,".pdf"), width = 12, height = 12 )
...
for (k in 1:length(FC)){
  if (k==1) main="F=0" else if (k==2) main="Constant 2019 catch" else main=paste0("Fscenario=",k-2)
...
for (t in t1:length(FC[[1]])){
  plot(x=risk.val[1,2:ncol(risk.val),t],y=risk.val[2,2:ncol(risk.val),t],pch=1:(length(FC)),xlim=c(0,1),ylim=c(0,max(risk.val[2,,])*1.1),xaxt="n",yaxt="n", col=1:1:ncol(risk.val))
  text(x=0.5,y=max(risk.val[2,,]),labels = years[t],cex=2)
  if(t==t1) legend("topleft",legend=0:(length(FC)-2),pch=1:(length(FC)),bty="n", col=1:ncol(risk.val))
  if(t==t1) axis(2)
...
for (k in 1:length(FC)){
  plot.new()
  if (k==1) label="Catch by fleet F=0" else if (k==2) label="Catch by fleet constant 2019" else label=paste0("Catch by fleet Fscenario=",k-2)
